...
<template>
  ...
  <div class="relative w-full">
    ...
    <div class="flex items-center w-full mt-2.5 mb-1" v-bind="$attrs">
      <input placeholder="+234" list="countryCodes" name="countryCode" id="countryCode" v-model="countryCode" class="pl-4 w-20 border-0 focus:outline-none focus:ring-0 placeholder:text-98A2B3 placeholder:text-sm placeholder:text-left placeholder:font-normal">
      <datalist id="countryCodes">
        <option v-for="code in options"> {{ code.dialing_code }}</option>
      </datalist>
      <div class="bg-E5E5E5 w-2.5 h-2.5"></div>
      <input :placeholder="placeholder" type="text" name="phoneNumber" inputmode="numeric" v-model="phoneNumber" class="placeholder:text-98A2B3 placeholder:text-sm placeholder:text-left placeholder:font-normal border-0 focus:outline-none text-sm text-1E1D24 focus:ring-0 h-12 px-4 aria-[invalid=true]:border-error flex-grow" />
