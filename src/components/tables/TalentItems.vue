  ...
  <div v-if="invitedList?.data?.length">
      ...
      >
        ...
        <template v-slot:action="data">
          <p class="bg-gradient-to-br from-orange to-red p-px rounded-lg w-fit ">
            <BaseButton type='button' @click="$router.push({ name: 'talents.campaign.show', params: { id: data.value?.campaign?.id} })" class='revoke-invite bg-white text-xs rounded-lg max-h-8'>
    ...
    <div class="divide-y md:hidden">
      <div v-for="(item, index) in formated" :key="index" class="py-2 flex space-x-4 items-center">
        ...
        <BaseButton type='button' @click="$router.push({ name: 'talents.campaign.show', params: { id: item?.campaign?.id} })" class='revoke-invite bg-white text-xs rounded-lg max-h-8 flex-none px-0'>
        <span class="flex items-center space-x-1">
...
const { invitedList } = storeToRefs(talentStore)

const name = computed( () => {
  return `${invitedList.value?.first_name} ${invitedList.value?.last_name}`
})
...
const formated = computed( () => {
  return invitedList.value?.data.map( (item) => {
    return {
...
const table = reactive({
  ...
  rows: formated,
  totalRecordCount: invitedList.value?.data?.length,
  sortable: {
    order: "created_at",
    sort: "asc",
  },
