          ...
          <div class="space-y-1 relative">
            ...
            <div v-if="hasAttribute">
              <div
                ...
                :key="field.key"
              >
                <div class="flex items-center space-x-4 pt-5 max-w-lg">
                  <!-- Location -->
                  <br />
                  <BaseSelect
                    label="Variant name"
                  ...
                  >
                    <span class="rounded-full border border-orange w-6 h-6 flex items-center justify-center text-orange">
                      <Icon name="minus" />
                    </span>
                  </button>
                </div>
            ...
            >
              <span class="rounded-full border border-orange w-6 h-6 flex items-center justify-center">
                <Icon name="add" />
              </span>
              <span>Add new variant</span>
            </button>
...
const schema = computed(() => {
  return object({
    ...
    quantity: number().typeError("This field should be a number").required().label("Quantity"),
    price: number().typeError("This field should be a number").required().label("Price"),
...
const submit = handleSubmit( async (values, actions) => {
  ...
  exclusiveStore.store({
    ...
    await router.push({name: "exclusives.index"});
  }).catch((error) => {
    Toast.fire({
      icon: "error",
